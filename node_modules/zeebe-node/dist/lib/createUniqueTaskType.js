...
function createUniqueTaskType({ bpmnFilePath, taskTypes, messages, }) {
    const bpmn = (0, fs_1.readFileSync)(bpmnFilePath, 'utf8');
    const newTaskTypes = taskTypes.map(t => ({ [t]: (0, uuid_1.v4)() }));
    const newMessages = messages.map(m => ({ [m]: (0, uuid_1.v4)() }));
    ...
    const newProcessId = `process-${(0, uuid_1.v4)()}`;
